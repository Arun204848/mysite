   ...
   Add/Remove Input fields
    ...
    $(document).ready(function() {
      $("#addmore").click(function() {
        $("#req_input").append('<div class="required_inp row"><div class="form-group col-xl-6"><input name="fname" class="form-control" placeholder="First Name" type="text"></div><div class="form-group col-xl-6"><input name="lname" class="form-control" placeholder="Last Name" type="text"></div>' + '<div class="remove_field"><input type="button" class="inputRemove" value="Remove"/><i class="fas fa-times"></i></div></div>');
      });
      $('body').on('click','.inputRemove',function() {
        $(this).parent().parent('div.required_inp').remove()
      });
    });
...
// language toogle
 ...
 $(document).ready(function() {
    $('.toogle_text').click(function() {
      // Remove 'active' class from all toggle options
      $('.toogle_text').removeClass('active');
...
$('#donate-form-btn').on('click', function (e) {
    ...
    var isValid = $('#donate-form').parsley().validate(); // Triggers Parsley validation
    
    if (isValid) {
        // Perform submit actions if validation is successful
        $('#donate-form').submit(); // Submit the form if everything is valid
    }
});
...
//     document.querySelectorAll('.navbar-nav .menu-item a').forEach(function(link) {
...
// });
 ...
 jQuery(document).ready(function($) {
  // Get the full current URL and remove any trailing slash
  var currentUrl = window.location.href.replace(/\/$/, ""); 

  // Loop through each menu item
  jQuery('.menu-item a').each(function() {
    // Get the full URL from the menu item and normalize it
    var menuItemUrl = $(this).prop('href').replace(/\/$/, ""); 

    // Check if the current URL exactly matches the menu item's URL
 ...
 $(document).ready(function () {
    $('.showrooms').on('click', function () {
      $('#room-booked').show(); // Toggle visibility
    });
  });

$(document).ready(function () {
    $('.booking-btn').on('click', function () {
      $('.guest_room_book').hide(); // Toggle visibility
       $('.gues_house_des').hide(); // Toggle visibility
      $('.personal_deatail_div').show(); // Toggle visibility
      $ ('.donation-form').hide();
    });
  });
